...
unsigned int ClassicGetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if ((pindexLast->nHeight+1) % params.ClassicDifficultyAdjustmentInterval() != 0)
    {
        if (params.fPowAllowMinDifficultyBlocks)
        {
            ...
            else
            {
                ...
                while (pindex->pprev && pindex->nHeight % params.ClassicDifficultyAdjustmentInterval() != 0 && pindex->nBits == nProofOfWorkLimit)
                    pindex = pindex->pprev;
                return pindex->nBits;
            }
        }
    ...
    if ((pindexLast->nHeight+1) != params.ClassicDifficultyAdjustmentInterval())
        blockstogoback = params.ClassicDifficultyAdjustmentInterval();
...
unsigned int ClassicCalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    if (nActualTimespan < params.nClassicPowTargetTimespan/4)
        nActualTimespan = params.nClassicPowTargetTimespan/4;
    if (nActualTimespan > params.nClassicPowTargetTimespan*4)
        nActualTimespan = params.nClassicPowTargetTimespan*4;
    ...
    if (fShift)
        bnNew >>= 1;
    bnNew *= nActualTimespan;
    bnNew /= params.nClassicPowTargetTimespan;
    if (fShift)
        bnNew <<= 1;
