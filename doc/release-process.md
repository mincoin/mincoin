...
##perform gitian builds
 ...
 Fetch and build inputs: (first time, or when dependency versions change)
	...
	mv build/out/mincoin-deps-0.0.5.zip inputs/
 ...
 Build mincoind and mincoin-qt on Linux32, Linux64, and Win32:
	...
	pushd build/out
	zip -r mincoin-${VERSION}-linux-gitian.zip *
	mv mincoin-${VERSION}-linux-gitian.zip ../../
	popd
	...
	pushd build/out
	zip -r mincoin-${VERSION}-win32-gitian.zip *
	mv mincoin-${VERSION}-win32-gitian.zip ../../
	popd
...
**Perform Mac build:**
  ...
  OSX binaries are created by Gavin Andresen on a 32-bit, OSX 10.6 machine.
	...
	qmake RELEASE=1 USE_UPNP=1 USE_QRCODE=1 mincoin-qt.pro
	make
...
* update wiki changelog: [https://en.mincoin.it/wiki/Changelog](https://en.bitcoin.it/wiki/Changelog)
...
From a directory containing mincoin source, gitian.sigs and gitian zips

	export VERSION=0.5.1
	mkdir mincoin-${VERSION}-linux-gitian
	pushd mincoin-${VERSION}-linux-gitian
	unzip ../mincoin-${VERSION}-linux-gitian.zip
	mkdir gitian
	cp ../mincoin/contrib/gitian-downloader/*.pgp ./gitian/
	for signer in $(ls ../gitian.sigs/${VERSION}/); do
	 cp ../gitian.sigs/${VERSION}/${signer}/mincoin-build.assert ./gitian/${signer}-build.assert
	 cp ../gitian.sigs/${VERSION}/${signer}/mincoin-build.assert.sig ./gitian/${signer}-build.assert.sig
	done
	zip -r mincoin-${VERSION}-linux-gitian.zip *
	cp mincoin-${VERSION}-linux-gitian.zip ../
	popd
	mkdir mincoin-${VERSION}-win32-gitian
	pushd mincoin-${VERSION}-win32-gitian
	unzip ../mincoin-${VERSION}-win32-gitian.zip
	mkdir gitian
	cp ../mincoin/contrib/gitian-downloader/*.pgp ./gitian/
	for signer in $(ls ../gitian.sigs/${VERSION}-win32/); do
	 cp ../gitian.sigs/${VERSION}-win32/${signer}/mincoin-build.assert ./gitian/${signer}-build.assert
	 cp ../gitian.sigs/${VERSION}-win32/${signer}/mincoin-build.assert.sig ./gitian/${signer}-build.assert.sig
	done
	zip -r mincoin-${VERSION}-win32-gitian.zip *
	cp mincoin-${VERSION}-win32-gitian.zip ../
	popd
